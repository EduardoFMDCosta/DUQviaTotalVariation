...
def plot_interval(gmm_prob_set: torch.Tensor,
    ...
    plt.plot(t, gmm_prob_set + alpha_set, label=r'$\hat{\mathbb{P}}_{x_t}(U) + \alpha_{U, t}$', marker='o')
    plt.plot(t, gmm_prob_set + beta_set, label=r'$\hat{\mathbb{P}}_{x_t}(U) + \beta_{U, t}$', marker='o')

    if actual_set_prob is not None:
        plt.plot(t, actual_set_prob, label=r'$\mathbb{P}_{x_t}(U)$', linestyle='-', marker='s', color='black')
